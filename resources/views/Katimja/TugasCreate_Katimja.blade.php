    ...
    <div class="card bg-white shadow-lg rounded-lg p-6">
        ...
        <form action="{{ route('tugas.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="task-title" id="task-title" class="block w-full mt-2 p-3 border border-gray-300 rounded-lg" required>
            </div>
            <div class="mb-4">
                <label for="task-desc" class="text-sm font-semibold text-gray-700">Deskripsi Tugas</label>
                <textarea name="task-desc" id="task-desc" class="block w-full mt-2 p-3 border border-gray-300 rounded-lg" required></textarea>
            </div>
            <div class="mb-4">
                <label for="deadline" class="text-sm font-semibold text-gray-700">Tanggal Deadline</label>
                <input type="date" name="deadline" id="deadline" class="block w-full mt-2 p-3 border border-gray-300 rounded-lg" required>
            </div>
            <button type="submit" class="mt-4 inline-block px-6 py-2 bg-green-600 text-white rounded-full shadow-md hover:bg-green-700">Kirim Tugas</button>
        </form>
    </div>
@endsection
