...
@section('title', 'Beranda Staff - SIOPLAS')
...
<div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-slate-200">
  <div>
    <h1 class="text-2xl font-bold text-slate-800 tracking-tight">
      Selamat Datang, {{ session('user_nama', 'Pegawai') }}
    </h1>
    <p class="text-sm text-slate-500 mt-1">
      Dashboard Pegawai: Pantau tugas yang diterima dan status pengiriman laporan.
    </p>
  </div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
  ...
  <div class="lg:col-span-2 space-y-4">
    <h2 class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Ringkasan Aktivitas Saya</h2>

    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center justify-between mb-3">
          <span class="text-xs font-medium text-slate-500">Laporan Terkirim</span>
      ...
      <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
        <div class="flex items-center justify-between mb-3">
          <span class="text-xs font-medium text-slate-500">Tugas Diterima</span>
          <span class="p-2 rounded bg-slate-100 text-slate-600">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  <div class="bg-slate-800 rounded-xl p-5 text-white shadow-sm flex flex-col justify-between border border-slate-700">
    <div>
      ...
      <textarea id="notepad-content"
        ...
        placeholder="Tulis pengingat tugas atau catatan harian Anda..."></textarea>
    </div>
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5 pb-3 border-b border-slate-100">
    <div>
      <h2 class="text-base font-bold text-slate-800">Daftar Tugas Staff</h2>
      <p class="text-xs text-slate-500 mt-0.5">Daftar tugas yang diberikan beserta batas waktunya.</p>
    </div>
        ...
        <tr class="hover:bg-slate-50 transition-colors">
          ...
          <td class="py-3 px-4 text-center">
            @php
              ...
              if($task['status'] == 'Menunggu Approval') $badgeClass = 'bg-blue-50 text-blue-700 border-blue-200';
            @endphp
